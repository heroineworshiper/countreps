...
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
        ...
        const Array<T>& pairScores)
    {
        try
        {
//            if (poseModel != BODY_25 && poseModel != COCO_18
//                && poseModel != MPI_15 && poseModel != MPI_15_4)
...
// printf("removePeopleBelowThresholdsAndFillFaces %d peopleVector=%d minSubsetCnt=%f personScore=%f personCounter=%f minSubsetScore=%f\n",
// __LINE__,
// (int)peopleVector.size(),
// (float)minSubsetCnt,
// (float)personScore,
// (float)personCounter,
// (float)minSubsetScore);
// peopleVector=1 minSubsetCnt=3.000000 personScore=19.698372 personCounter=12.000000 minSubsetScore=0.400000
// peopleVector=1 minSubsetCnt=3.000000 personScore=1.690333 personCounter=2.000000 minSubsetScore=0.400000
...
//                              const T* const peaksPtr, const int poseModel, const Point<int>& heatMapSize,
...
//     }
    ...
    void connectBodyPartsCpu(
        ...
        const int poseModel, const Point<int>& heatMapSize, const int maxPeaks, const T interMinAboveThreshold,
...
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    template  void connectBodyPartsCpu(
        Array<float>& poseKeypoints, Array<float>& poseScores, const float* const heatMapPtr,
        const float* const peaksPtr, const int poseModel, const Point<int>& heatMapSize, const int maxPeaks,
    ...
    template  void connectBodyPartsCpu(
        ...
        const double* const peaksPtr, const int poseModel, const Point<int>& heatMapSize, const int maxPeaks,
    ...
    template  std::vector<std::pair<std::vector<int>, float>> createPeopleVector(
        const float* const heatMapPtr, const float* const peaksPtr, const int poseModel,
    ...
    template  std::vector<std::pair<std::vector<int>, double>> createPeopleVector(
        const double* const heatMapPtr, const double* const peaksPtr, const int poseModel,
