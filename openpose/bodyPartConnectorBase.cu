...
namespace op
{
    ...
    inline __device__  T process(
        ...
        const T* mapX, 
        const T* mapY, 
        const int heatmapWidth,
        const int heatmapHeight, 
    ...
    __global__ void pafScoreKernel(
        ...
        const unsigned int maxPeaks, 
        const int numberBodyPartPairs,
        const int heatmapWidth, 
        const int heatmapHeight, 
        const T interThreshold, 
        const T interMinAboveThreshold,
...
// printf("connectBodyPartsGpu %d %d %d %d %d %f %f %f\n",
// __LINE__,
// maxPeaks, 
// (int)numberBodyPartPairs, 
// heatMapSize.x, 
// heatMapSize.y, 
// interThreshold,
// interMinAboveThreshold, 
// defaultNmsThreshold);
// connectBodyPartsGpu 233 127 26 256 144 0.050000 0.950000 0.050000
            ...
            pafScoreKernel<<<numBlocks, THREADS_PER_BLOCK>>>(
                ...
                defaultNmsThreshold);
            // pairScoresCpu <-- pairScoresGpu
            cudaMemcpy(pairScoresCpu.getPtr(), 
                pairScoresGpuPtr, 
                totalComputations * sizeof(T),
                cudaMemcpyDeviceToHost);
            ...
            const auto pairConnections = pafPtrIntoVector(
                ...
                bodyPartPairs, 
                numberBodyPartPairs);
// printf("connectBodyPartsGpu %d maxPeaks=%d numberBodyPartPairs=%d totalComputations=%d pairConnections.size()=%d\n",
// __LINE__,
// maxPeaks,
// numberBodyPartPairs,
// totalComputations,
...
// printf("connectBodyPartsGpu %d peopleVector.size()=%d %f\n",
// __LINE__,
// peopleVector.size(),
// (float)peopleVector[0].second);
            removePeopleBelowThresholdsAndFillFaces(
                ...
                maximizePositives, 
                peaksPtr);
// printf("connectBodyPartsGpu %d validSubsetIndexes.size()=%d\n",
// __LINE__,
// validSubsetIndexes.size());
...
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    template void connectBodyPartsGpu(
        ...
        const float* const heatMapGpuPtr,
        ...
        const bool maximizePositives, 
        Array<float> pairScoresCpu, 
        float* pairScoresGpuPtr,
        const unsigned int* const bodyPartPairsGpuPtr, 
        const unsigned int* const mapIdxGpuPtr,
        const float* const peaksGpuPtr);
    template void connectBodyPartsGpu(
        Array<double>& poseKeypoints, 
        Array<double>& poseScores, 
        const double* const heatMapGpuPtr,
        const double* const peaksPtr, 
        const int poseModel, 
        const Point<int>& heatMapSize, 
        const int maxPeaks,
        const double interMinAboveThreshold, 
        const double interThreshold, 
        const int minSubsetCnt,
        const double minSubsetScore, 
        const double scaleFactor, 
        const double defaultNmsThreshold,
        const bool maximizePositives, 
        Array<double> pairScoresCpu, 
        double* pairScoresGpuPtr,
        const unsigned int* const bodyPartPairsGpuPtr, 
        const unsigned int* const mapIdxGpuPtr,
