...
//         {F135+68, "RPupil"}, {F135+69, "LPupil"}, // Pupils
    ...
    const std::map<unsigned int, std::string> POSE_BODY_PART_MAPPING[] = {
    ...
    const std::string POSE_PROTOTXT[] = {
    ...
    const std::string POSE_TRAINED_MODEL[] = {
    ...
    const unsigned int POSE_NUMBER_BODY_PARTS[] = {
    ...
    const std::vector<unsigned int> POSE_BODY_PART_PAIRS[] = {
        // BODY_25
        std::vector<unsigned int>{
...
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    unsigned int getPoseNumberBodyParts(const int poseModel)
    {
        try
        {
            return POSE_NUMBER_BODY_PARTS[(int)poseModel];
        }
        catch (const std::exception& e)
        {
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
...
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    float getPoseNetDecreaseFactor(const int poseModel)
    {
        try
        {
            return (poseModel != BODY_19_X2 ? 8.f : 4.f);
        }
        catch (const std::exception& e)
        {
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
...
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    float getPoseDefaultNmsThreshold(const int poseModel, const bool maximizePositives)
    {
        try
        {
            // MPI models
...
//            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    float getPoseDefaultConnectInterThreshold(const int poseModel, const bool maximizePositives)
    {
        try
        {
            // MPI models
            if (poseModel == MPI_15 || poseModel == MPI_15_4)
                return 0.01f;
            // Non-MPI models
            else
